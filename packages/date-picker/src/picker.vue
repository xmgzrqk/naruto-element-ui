...
const TYPE_VALUE_RESOLVER_MAP = {
  ...
  weekrange: {
    formatter(value, format) {
      const start = new Date(value[0].getTime() + 1000 * 60 * 60 * 24);
      const end = new Date(value[1].getTime() - 1000 * 60 * 60 * 24 * 5);
      const formatStartDate = TYPE_VALUE_RESOLVER_MAP.week.formatter(start, format);
      const formatEndDate = TYPE_VALUE_RESOLVER_MAP.week.formatter(end, format);
      return [formatStartDate, formatEndDate];
    },
    parser(value, format) {
      return (typeof value === 'string' ? value.split(', ') : value)
        .map(date => date instanceof Date ? date : TYPE_VALUE_RESOLVER_MAP.date.parser(date, format));
    }
  },
  quarter: {
    formatter(value, format) {
      ...
      date = /qq/.test(date)
        ? date.replace(/qq/, quarter < 10 ? '0' + quarter : quarter)
        : date.replace(/q/, quarter);
      return date;
    },
    parser(text, format) {
      // parse as if a normal date
  ...
  quarterrange: {
    formatter(value, format) {
      return value.map(date => TYPE_VALUE_RESOLVER_MAP.quarter.formatter(date, format));
    },
    parser(value, format) {
      return (typeof value === 'string' ? value.split(', ') : value)
        .map(date => date instanceof Date ? date : TYPE_VALUE_RESOLVER_MAP.date.parser(date, format));
